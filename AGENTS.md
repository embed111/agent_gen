...
## 0. OpenClaw Workspace Boundary

开工时先读取并遵守以下本地记忆顺序：

1. `.codex/SOUL.md`
2. `.codex/USER.md`
3. 近两天 `.codex/memory/*.md`
4. `.codex/MEMORY.md`

本工作区额外边界如下：

1. 顶层 Agent 工作文档只保留 `AGENTS.md`。
2. `.codex/*` 只记录生成经验、稳定约束、失败模式。
3. 正式报告、测试、模板、脚本继续留在既有目录，不写入 `.codex/`。
...
13. **最终输出**：生成完整 `AGENTS.md` 内容（仅在生成模式）。
14. **附加自检**：输出“配置覆盖率报告”。
